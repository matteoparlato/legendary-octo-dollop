 ...
 */
async function validateDrive(driveId: string, client: MicrosoftGraph.Client): Promise<string> {
    if(!driveId) {
        throw new Error('Parameter driveId is required.');
    }

    console.log(`Validating drive id / url: '${driveId}'.`);
    ...
    console.log(`Found drive '${drive?.name}'.`);
